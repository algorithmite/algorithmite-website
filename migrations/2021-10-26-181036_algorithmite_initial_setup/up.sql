...
CREATE TYPE user_action_types AS ENUM ('login', 'logout', 'create', 'delete', 'reset_password_logged_in', 'reset_password_email_sent', 'reset_password_email');

CREATE TABLE user_actions (
  user_action_key serial PRIMARY KEY,
  actor integer REFERENCES users NOT NULL,
  ip inet,
  action user_action_types NOT NULL,
  created_at timestamp NOT NULL DEFAULT NOW()
);

CREATE TYPE moderation_action_types AS ENUM ('change_role', 'delete_comment', 'undo_delete_comment');
...
CREATE TABLE moderation_actions (
  ...
  action moderation_action_types NOT NULL,
  created_at timestamp NOT NULL DEFAULT NOW()
);
